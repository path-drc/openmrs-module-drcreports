...
import static org.openmrs.module.drcreports.reports.PatientHistoryReportManager.REPORT_DESIGN_UUID;
...
import org.openmrs.module.drcreports.common.DRCReportsPrivilegeConstants;
...
public class PatientHistoryPdfReport {
	...
	public byte[] getBytes(Patient patient, Set<Encounter> encounters)
		...
		if (patient != null) {
			Context.requirePrivilege(DRCReportsPrivilegeConstants.VIEW_PATIENT_HISTORY);
		...
		StreamSource xslTransformStream = new StreamSource(
		        OpenmrsClassLoader.getInstance().getResourceAsStream(DRCReportsConstants.PATIENT_HISTORY_XSL_PATH));
